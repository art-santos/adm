...
export default function PopupEditPlans({provider, internetspeed, id, title, mainPrice, dspeed, dhighlight1, dhighlight2, dhighlight3, dhighlight4, dtype}) {
  const classes = useStyles();
  const {open, setOpen} = useContext(EditPlanContext)
  const [file, setFile] = useState(""); 
  const [display, setDisplay] = useState("none")
  const [name, setName] = useState(title);
  const [price, setPrice] = useState(mainPrice);
  const [speed, setSpeed] = useState(internetspeed || 'no internet available');
  const [speedTitle, setSpeedTitle] = useState(dspeed || 'no internet available')
  const [load, setLoad] = useState(false);
  const [type, setType] = useState(dtype);
  const [highlight1, setHighlight1] = useState(dhighlight1);
  const [highlight2, setHighlight2] = useState(dhighlight2);
  const [highlight3, setHighlight3] = useState(dhighlight3);
  const [highlight4, setHighlight4] = useState(dhighlight4);
 ...
 const handleType = (e) => {
  ...
  return (  
      <>
        ...
        <DialogTitle id="form-dialog-title" style={{ textAlign: "center"}}>{provider.toUpperCase()+ ' - ' +title.toUpperCase()}</DialogTitle>
        ...
        <Button color="primary" variant="contained"  type="submit" style={{marginBottom:"15px"}}>
          Submit
        </Button>
